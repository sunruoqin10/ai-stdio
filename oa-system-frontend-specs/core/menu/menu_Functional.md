...
- [x] 树形表格展示菜单层级
- [x] 支持展开/收起子菜单
- [x] 显示菜单名称、图标、路由、类型、排序、状态等信息
- [x] 右键菜单操作(新增子菜单、编辑、删除、启用/禁用)
- [x] 搜索菜单(按名称、路由)
- [x] 筛选(按类型、状态)
- [x] 导出菜单配置
- [x] 一键初始化系统菜单
...
- [x] 检测菜单初始化状态
- [x] 一键初始化系统菜单
- [x] 预置菜单数据(员工、部门、资产、请假、报销、会议、权限、字典等)
- [x] 菜单编码自动生成(MENU_001格式)
- [x] 父子关系自动建立
- [x] 初始化状态持久化(localStorage)
- [x] 支持重置初始化标记
...
#### 4.2.6 初始化系统菜单 ⭐ NEW
1. 进入菜单管理页面
2. 系统检测是否已初始化(检查localStorage标记)
3. 未初始化时显示"初始化系统菜单"按钮
4. 点击按钮,弹出确认对话框
5. 确认后批量创建预置菜单:
   - 系统管理(目录)
   - 员工名录
   ...
   - 角色管理(子菜单)
   - 权限管理(子菜单)
6. 自动生成菜单编码(MENU_001, MENU_002...)
7. 自动建立父子关系
8. 设置初始化标记到localStorage
9. 刷新菜单列表显示
...
#### 5.3.1 权限检查优化
- **开发环境**: 自动禁用所有权限检查,默认返回true
- **生产环境**: 严格按权限码验证
- **检测方式**: 通过 `import.meta.env.DEV` 判断

#### 5.3.2 数据加载策略
- **开发环境**: 使用Mock数据,支持实时修改
- **生产环境**: 调用真实API接口
- **Mock数据位置**: `src/modules/menu/mock/data.ts`

#### 5.3.3 数据持久化
- 菜单数据持久化到localStorage
- 缓存键: `oa_menu_data`
- 初始化标记: `oa_menu_initialized`
- 支持手动清除缓存重新初始化
